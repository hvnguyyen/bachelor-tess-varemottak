...
- Proxy-route finnes
- Cloudflare hindret oss i å proxy-route server-side, så flyten er implementert med klientkall fra frontend via en klientbasert adapter
...
- QR er ikke verifisert, og scanner fungerer foreløpig kun som en strekkode-prototype
- Kamera lastes heller ikke automatisk, bruker må først åpne Varemottak fra Dashboard, og deretter åpne skanning manuelt
...
#7. Tillegg / annet:
- Bekreftelsesmodul før `POST` oppsummerer mottaket og fungerer kun som 'submit-confirmation'
- En enkel historikkside med `localStorage` viser lokalt lagrede mottak på denne enheten/nettleseren
- `customerNumber` er ikke lenger hardkodet, men hentes fra innlogget brukerprofil via `/user`
- Brukerprofil lagres i en egen helper (`lib/userProfile`) og brukes videre i dashboard og varemottak
- Flyten er ferdig implementert, men ved behov kan UX og UI forbedres i en senere iterasjon
...
4. Før ordredata kan brukes i `Varemottak`, hentes relevant `customerNumber` fra innlogget brukerprofil
5. Hvis bruker velger `Hent aktuelle ordredata`:
    - frontend henter ordredata fra `GET https://api.tessix.no/order/{customerNumber}`
    - `customerNumber` kommer fra innlogget brukerprofil og er ikke lenger hardkodet
    - ordre vises som komprimert tabell med én rad per ordrenummer
    - bruker kan filtrere og åpne detaljvisning for ordrelinjer
6. Hvis bruker velger `Åpne kamera for mottak av kolli`:
...
6. Når bruker klikker `Registrer mottak`:
    ...
    - bruker må eksplisitt bekrefte før innsending
...
9. Appen:
    ...
    - lagrer mottaket lokalt i `localStorage` sammen med `customerNumber` og brukeridentitet
...
Da Cloudflare behandler serverkallet annerledes enn browserkallet, og ettersom dette hindrer server-side proxying så vi oss nødt til å implementere enda et mellom-/integrasjonslag i form av en klientadapter hvor frontend kaller TESS API `https://api.tessix.no/order/{customerNumber}` (tidligere `https://api.tessix.no/order/169999`) direkte.

# 4.3 Scannefunksjon med kamera
Foreløpig er QR-kode skanning ikke verifisert, men strekkoder registereres og utelukker duplikater med feilmelding.

# 4.4 Kundegrunnlag fra innlogget bruker
I første versjon av fase 2 ble ordreendepunktet testet mot hardkodet `customerNumber = 169999`. Senere ble dette endret slik at `defaultCustomerNumber` og `customerNumbers` hentes fra innlogget brukerprofil via `/user` og lagres i en egen helper (`lib/userProfile`). På denne måten kunne ordregrunnlaget knyttes til faktisk innlogget bruker i stedet for en fast testverdi.

## 5. Presise avgrensninger (kjente begrensninger)
- Flyten for varemottak er avgrenset til en demonstrerbar prototype, ikke en full backend-integrert mottaksløsning. 
- Ordredata hentes fra ekte TESS-endepunkt, men vises i en forenklet og brukerorientert tabellvisning. 
- Selve mottaket skjer via intern mock-route med lokal historikk i localStorage, og ikke som endelig ERP-integrasjon. 
- Løsningen demonstrerer scanning, oppsummering og innsending, men ikke full produksjonsflyt på tvers av backend-systemer.
- Kundegrunnlaget hentes nå fra innlogget brukerprofil, men støtte for flere kundenumre er foreløpig enkel og ikke fullt ut videreutviklet som egen arbeidsflyt.
- Historikk over mottak lagres kun lokalt i nettleseren på samme enhet og er ikke en delt backend-basert historikkløsning.
...
- Ordredata kan hentes fra `GET https://api.tessix.no/order/{customerNumber}` i klientkontekst med gyldig sesjons-cookie
- Relevant `customerNumber` hentes fra innlogget brukerprofil i stedet for hardkodet testverdi
...
## 7. Retrospekt / Refleksjonsnotat (til sluttrapport)
...
I en senere iterasjon ble ordregrunnlaget også knyttet til innlogget brukerprofil i stedet for hardkodet `customerNumber`. Dette gjorde løsningen mer konsistent med faktisk brukerdata fra TESS og reduserte behovet for manuelle testantakelser i varemottaksflyten.
